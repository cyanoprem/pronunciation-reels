...
- **Base URL**: `https://app.gosupernova.live` (`app.getsupernova.ai` mirrors the same API). Earlier drafts of `docs/api-web-prototypes.md` referenced `app.gosupernova.com`, which is NXDOMAIN — never use it.
- **KV endpoints have no CORS**. `/api/kv/*` and `/api/users/kv/*` return no `Access-Control-*` headers, so direct browser fetches from `*.vercel.app` or `*.gosupernova.live` get blocked at preflight. Route every client KV call through the same-origin Next.js proxy at `app/api/kv-proxy/[key]/route.ts`. It forwards `Authorization`, `x-sn-user-id`, and all `x-sn-*` headers to the upstream KV endpoint. Pattern: client fetches `/api/kv-proxy/{encodeURIComponent(key)}`, never the cross-origin upstream directly. See the "CORS limitation" section in `docs/api-web-prototypes.md`.
