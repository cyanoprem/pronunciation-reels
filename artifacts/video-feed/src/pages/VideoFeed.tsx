...
import sparkleIcon from "@assets/weavy-Vectorizer-2026-02-02_at_17.48.29_2_1778749310651.svg";
...
function SparkleBadge({ size = 31 }: { size?: number }) {
  ...
  return (
    ...
    >
      <img src={sparkleIcon} alt="" width={inner} height={inner} style={{ display: "block" }} />
    </span>
  );
}

function VideoCard({ card, muted, onToggleMute }: { card: VideoCard; muted: boolean; onToggleMute: () => void }) {
    ...
    <div ref={containerRef} className="snap-item" style={{ background: "#0D0E1A" }} data-testid={`video-card-${card.id}`}>
      ...
      <div className="absolute left-5 right-5 bottom-10 z-10 flex items-start justify-between gap-4 pointer-events-none">
        <div className="flex flex-col items-start gap-3 pointer-events-auto">
          ...
          <button
            onClick={() => setLocation(`/practice?word=${encodeURIComponent(card.word)}&phonetic=${encodeURIComponent(card.phonetic)}`)}
